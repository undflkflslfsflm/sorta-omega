...
export const calendarOverlayItemSchema=z.object({id:idSchema,kind:z.enum(["lesson","assignment","assessment","attendance"]),title:z.string(),startsAt:z.string().datetime(),endsAt:z.string().datetime().nullable(),layer:z.enum(["school","deadline","assessment","attendance"]),attendanceStatus:z.enum(["present","absent","late","unknown"]).nullable().default(null),reason:z.literal("source_record_not_calendar_event"),revision:z.number().int().positive()});
