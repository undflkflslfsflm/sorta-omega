...
| Remove manual “cockpit” work from everyday use; commitments should come from capture/import | Local implementation: primary Tasks screen has no add forms; explicit Inbox task commands create source-linked tasks automatically, and inferred task notes offer one-step acceptance. Person-linked promise extraction and Brain simplification are still missing | Push and deploy; live-test explicit capture, inferred-note acceptance, duplicate prevention and task/source linkage; then build promise extraction and simplify remaining surfaces |
