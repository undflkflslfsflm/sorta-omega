...
| Automatically populate subjects, courses, lessons, assessments, assignments, attendance, grades and their details from InSchool/Teams | Partial: timetable one-week import live-verified; broader traversal and other categories incomplete. Snapshot import now has source-linked attendance and grade storage, but no browser collection for them | Reauthenticate the 4090's isolated school browser, verify multiweek import, then add each missing source dataset with coverage checks |
...
| Norwegian/European date display and entry | Local implementation: reminders, assignment due times, assessment dates, attendance dates, grade dates, and goal dates use explicit `dd.mm.yyyy` entry; timestamps include 24-hour `hh:mm` | Push and deploy; audit remaining date displays and verify on the live phone browser |
