import type { ActivityEvent, AiOperation, AiStatus, AskHandle, AttendanceRecord, AttendanceSummary, AutomationDecision, BlobSummary, Calendar as CalendarRecord, CalendarBrief, CalendarEntity, CalendarEvent, CalendarPolicySet, CalendarView, Capture, Chat, ChatMessage, Collection, Commitment, CommitmentDetail, EditorDocument, EventReminderPlan, ExportManifest, Flashcard, FlashcardDeck, FlashcardReview, FlashcardReviewItem, Goal, Idea, IndexStatus, Job, JobHandle, KnowledgeGap, Label, Memory, ModelProfile, MomentumPreferences, MomentumSummary, NextActionSet, Note, NoteCommitmentCandidate, NoteRevision, Notification, OccurrenceException, PerformanceGrade, PerformanceSummary, PerformanceTarget, PersonalProfile, Preferences, PrepItem, Project, Proposal, ProposalUndoReceipt, ProviderCalendarAction, RelatedResult, Relationship, Reminder, RoutingRule, RulePreviewResult, ScheduleExplanation, SchedulerPreferences, SchoolAssessment, SchoolAssignment, SchoolCourse, SchoolLesson, SchoolSubject, SearchResult, SourceObjectDetail, StudyAttempt, StudyExercise, StudyPlan, StudySession, SystemStatus, Task, TaskExecutionHistory, Today, TodayNextActionPlan, UndoReceipt, UploadSession } from "@sorta/contracts";
...
export const api = {
  ...
  authorizeGoogleGmail: (connection:IntegrationConnection) => request<{authorizationUrl:string}>(`${vaultPath}/connections/${connection.id}/authorize`,{method:"POST",body:JSON.stringify({requestedCapabilities:["mail.read"],registeredReturnTarget:"connections"})}),
  ...
  commitmentCandidates: () => request<{ items: NoteCommitmentCandidate[] }>(`${vaultPath}/commitment-candidates`),
  decideCommitmentCandidate: (candidate:NoteCommitmentCandidate,decision:"accept"|"dismiss") => request<NoteCommitmentCandidate>(`${vaultPath}/commitment-candidates/${candidate.id}/decision`,{method:"POST",body:JSON.stringify({expectedNoteRevision:candidate.noteRevision,decision})}),
  ...
  sourceObjectDetail: (sourceObjectId: string) => request<SourceObjectDetail>(`${vaultPath}/source-objects/${sourceObjectId}`),
