...
try{
  if(provider==="teams"){
    ...
    const selectors=['[data-tid="chat-pane-message"]','[data-tid="message-pane-list-runway"] [role="listitem"]','[role="log"] [role="listitem"]'];let texts:string[]=[];for(const selector of selectors){texts=await page.locator(selector).allTextContents();if(texts.length)break;}if(!texts.length)throw new Error("teams_message_elements_not_found_layout_review_required");const capturedAt=new Date().toISOString();await mkdir(path.dirname(destination),{recursive:true});const temporary=`${destination}.${process.pid}.tmp`;await writeFile(temporary,`${JSON.stringify(teamsNotes(texts,capturedAt),null,2)}\n`,{flag:"wx"});await rename(temporary,destination);console.log(JSON.stringify({provider:"teams",format:"omega_notes_json_v1",itemCount:teamsNotes(texts,capturedAt).length,output:destination,sessionRetainedInProfile:true,credentialsExported:false}));
  }else{
