...
export function cleanVisibleTexts(values:string[],limit=5_000){const seen=new Set<string>(),items:string[]=[];for(const value of values){const clean=value.replace(/\u00a0/g," ").replace(/[ \t]+/g," ").replace(/\n{3,}/g,"\n\n").trim();if(!clean||clean.length>100_000||seen.has(clean))continue;seen.add(clean);items.push(clean);if(items.length>=limit)break;}return items;}
