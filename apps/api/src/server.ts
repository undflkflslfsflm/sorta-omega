...
async function saveMicrosoftSource(vaultId:string,connectionId:string,source:GraphSource){
  const contentHash=createHash("sha256").update(source.content).update(JSON.stringify(source.metadata)).digest("hex");
  await transaction(async client=>{
    ...
    if(current){
      ...
      await client.query("INSERT INTO source_object_revisions(source_object_id,revision,content_hash,content_text,metadata) VALUES ($1,$2,$3,$4,$5::jsonb)",[current.id,next,contentHash,source.content,JSON.stringify(source.metadata)]);
    ...
    await client.query("INSERT INTO source_object_revisions(source_object_id,revision,content_hash,content_text,metadata) VALUES ($1,1,$2,$3,$4::jsonb)",[inserted.id,contentHash,source.content,JSON.stringify(source.metadata)]);
  });
}
