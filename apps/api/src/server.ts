...
async function saveMicrosoftSource(vaultId:string,connectionId:string,source:GraphSource){
...
app.post("/api/v1/vaults/:vaultId/connections/:connectionId/sync",async(request,reply)=>{
  ...
  if(!connection)return reply.code(404).send({error:"connection_not_found"});
  if(connection.provider!=="microsoft")return reply.code(409).send({error:"live_sync_adapter_not_bound"});
  const credential=(await query<Record<string,any>>("SELECT * FROM provider_oauth_credentials WHERE connection_id=$1 AND provider='microsoft'",[connectionId])).rows[0];
  ...
  if(credential.expires_at&&new Date(credential.expires_at).getTime()<Date.now()+60_000){
    const registration=oauthRegistration("microsoft");
    if(!registration||!tokens.refresh_token)return reply.code(409).send({error:"reauthorization_required",detail:"Request offline.access during Microsoft authorization."});
