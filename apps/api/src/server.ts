...
app.get("/api/v1/vaults/:vaultId/calendar-view",async(request,reply)=>{
  ...
  const localDate=(instant:string)=>new Intl.DateTimeFormat("en-CA",{timeZone:input.timezone,year:"numeric",month:"2-digit",day:"2-digit"}).format(new Date(instant));
  ...
  const projectSchoolRecord=(row:Record<string,any>,kind:"lesson"|"assignment"|"assessment",title:string,spec:Record<string,any>,layer:"school"|"deadline"|"assessment")=>{if(spec.kind==="exact"){const startsAt=spec.startsAt??spec.dueAt;const endsAt=spec.endsAt??null;if(startsAt&&Date.parse(startsAt)>=Date.parse(input.from)&&Date.parse(startsAt)<Date.parse(input.to))overlayItems.push({id:row.id,kind,title,startsAt,endsAt,layer,reason:"source_record_not_calendar_event",revision:row.revision});return;}if(spec.kind==="date_only"&&!dateIsVisible(spec.date))return;unknownTimeMarkers.push({id:row.id,kind,title,date:spec.kind==="date_only"?spec.date:null,reason:spec.kind==="date_only"?"date_only":"time_unknown",revision:row.revision});};
  ...
  for(const row of attendanceRows.rows){
    ...
    const linked=occurrences.find(occurrence=>occurrence.eventId===row.calendar_event_id&&localDate(occurrence.startsAt)===date);
    ...
    if(startsAt&&localDate(startsAt)===date)overlayItems.push({id:row.id,kind:"attendance",title,startsAt,endsAt,layer:"attendance",attendanceStatus:row.normalized_status,reason:"source_record_not_calendar_event",revision:row.revision});
