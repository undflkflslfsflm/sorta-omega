...
export function initialIntegration(provider:Provider):{state:State;capabilities:Capability[]}{
  ...
  const capability=provider==="google_calendar"?"google.gmail_read":provider==="microsoft"?"microsoft.selected_resources":provider==="firecrawl"?"public_web.fetch":`${provider}.owner_export`;
