...
describe("integration capability honesty",()=>{
  ...
  it("labels the Google read path as Gmail rather than claiming calendar sync",()=>{expect(initialIntegration("google_calendar").capabilities[0]?.key).toBe("google.gmail_read");});
