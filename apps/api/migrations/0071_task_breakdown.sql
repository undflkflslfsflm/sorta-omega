...
ALTER TABLE jobs ADD CONSTRAINT jobs_kind_check CHECK (kind IN (
  ...
  'transcript_analysis','url_capture','artifact_generate','performance_recommendations','task_breakdown'
));

UPDATE workers
SET allowed_job_types=array_append(allowed_job_types,'task_breakdown'),config_revision=config_revision+1,updated_at=now()
WHERE role='model' AND NOT ('task_breakdown'=ANY(allowed_job_types));
